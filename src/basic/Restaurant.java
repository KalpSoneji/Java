...
public class Restaurant {
    ...
    public static void printBill(int Khichdi, int Thepla, int Dhokla,
        ...
        System.out.println("\n====== BILL DETAILS ======");
        if (Khichdi > 0) {
            System.out.println("Khichdi x " + Khichdi + " = ₹" + (Khichdi * 50));
            total += Khichdi * 50;
        }
        if (Thepla > 0) {
            System.out.println("Thepla x " + Thepla + " = ₹" + (Thepla * 15));
            total += Thepla * 15;
        }
        if (Dhokla > 0) {
            System.out.println("Dhokla x " + Dhokla + " = ₹" + (Dhokla * 40));
            total += Dhokla * 40;
        }
        if (Sarso > 0) {
            System.out.println("Sarson Ka Saag x " + Sarso + " = ₹" + (Sarso * 70));
            total += Sarso * 70;
        }
        if (Makki > 0) {
            System.out.println("Makki Di Roti x " + Makki + " = ₹" + (Makki * 20));
            total += Makki * 20;
        }
        if (Chole > 0) {
            System.out.println("Chole Bhature x " + Chole + " = ₹" + (Chole * 50));
            total += Chole * 50;
        }
        if (Idli > 0) {
            System.out.println("Idli x " + Idli + " = ₹" + (Idli * 30));
            total += Idli * 30;
        }
        if (Dosa > 0) {
            System.out.println("Dosa x " + Dosa + " = ₹" + (Dosa * 80));
            total += Dosa * 80;
        }
        if (Vada > 0) {
            System.out.println("Vada x " + Vada + " = ₹" + (Vada * 40));
            total += Vada * 40;
        }
